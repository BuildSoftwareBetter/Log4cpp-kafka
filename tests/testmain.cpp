...
int main(int argc, char **argv)
{
	...
	// syslogAppender = new log4cpp::KafkaAppender("syslogdummy","localhost:9092","topic",1);
#endif

	if (argc < 2)
	{
		// appender = new log4cpp::OstreamAppender("default", &std::cout);
		appender = new log4cpp::KafkaAppender("syslogdummy", "localhost:9092", "topic", 1);
	}
	else
	{
	...
	sub2 << log4cpp::Priority::WARN << "warn2"
		 << " warn3"
		 << log4cpp::eol << " warn4";
