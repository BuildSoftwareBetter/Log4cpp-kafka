...
void SimpleConfigurator::configure(std::istream &initFile)
{
    ...
    while (initFile >> nextCommand)
    {
        ...
        log4cpp::Category &category =
            (categoryName.compare("root") == 0) ? log4cpp::Category::getRoot() : log4cpp::Category::getInstance(categoryName);
        ...
        if (nextCommand.compare("appender") == 0)
        {
            ...
            if (initFile >> layout >> appenderName)
            {
                ...
                else if (appenderName.compare("kafka") == 0)
                {
                    std::string brokers;
                    std::string topic;
                    ...
                    if (!(initFile >> brokers))
                    {
                        throw ConfigureFailure("Missing brokers for kafka for category: " + categoryName);
                    }
                    if (!(initFile >> topic))
                    {
                        throw ConfigureFailure("Missing kafka topic for category: " + categoryName);
                    }
                    if (!(initFile >> timeout))
                    {
                        throw ConfigureFailure("Missing timeout kafka for category: " + categoryName);
                    }
                    appender =
                        new log4cpp::KafkaAppender(categoryName, brokers, topic, timeout);
                }
#if LOG4CPP_HAVE_SYSLOG
